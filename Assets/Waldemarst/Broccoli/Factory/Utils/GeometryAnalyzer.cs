...
namespace Broccoli.Utils
{
    ...
    public class GeometryAnalyzer
    {
            ...
            bool isAdditive = true)
        {
            ...
            {
                _branch = branches[i];
                branchGirth = _branch.maxGirth;
                points = _branch.curve.GetPoints(resolutionAngle);
                for (int j = 0; j < points.Count; j++)
                {
                    point = points[j];
                    crossPoint = Vector3.Cross(point.forward.normalized, Vector3.right);
                    branchPos = _branch.GetPointAtPosition(point.relativePosition);
