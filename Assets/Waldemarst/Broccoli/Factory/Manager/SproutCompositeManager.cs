...
namespace Broccoli.Manager
{
    ...
    public class SproutCompositeManager
    {
        ...
        /// <summary>
        /// Gets a plane mesh for testing purposes.
        /// </summary>
        /// <param name="branchDescriptorId">Id for the branch descriptor (snapshot).</param>
        /// <param name="lod">Level of detail for the mesh.</param>
        /// <param name="useCache"><c>True</c> to use the cache.</param>
        /// <returns>Mesh for the branch descriptor / LOD.</returns>
        public Mesh GetDummyMesh(int branchDescriptorId, int lod, bool useCache = true)
        {
            Mesh mesh = MeshUtils.GetPlane(0.3f, 0.3f, 3, 3);
            return mesh;
        }
        /// <summary>
        ...
        private void ReflectIncludeAndExcludesToMesh(PolygonArea polygonArea, BranchDescriptor branchDescriptor)
        {
            ...
            for (int i = 0; i < uv5.Count; i++)
            {
                if (shownBranchIds.Contains((int)uv5[i].x))
                {
                    colors[i].a = 1f;
                }
                else
