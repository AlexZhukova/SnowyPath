...
namespace Broccoli.Factory
{
    ...
    /// ProcessSnapshotPolygons > GenerateSnapshotPolygonsPerLOD: analyzes and creates mesh data (polygons) for each branch descriptor (snapshot).
    /// </summary>
    public class SproutSubfactory
    {
        ...
        {
            if (branchDescriptorIndex < 0 || branchDescriptorCollection.branchDescriptors.Count == 0) return;
            ...
            for (int i = 0; i < branchDescriptor.sproutALevelDescriptors.Count; i++)
            {
                if (i < branchLevelCount)
                {
                    ...
                    sproutASL.normalSproutRandomness = branchDescriptor.sproutANormalRandomness;
            ...
            for (int i = 0; i < branchDescriptor.sproutBLevelDescriptors.Count; i++)
            {
                if (i < branchLevelCount)
                {
                    ...
                    sproutBSL.normalSproutRandomness = branchDescriptor.sproutBNormalRandomness;
        ...
        public void ProcessSnapshot(int snapshotIndex, bool clearCompositeManager = true, MaterialMode materialMode = MaterialMode.Composite, bool isNewSeed = false)
        {
            /*
            DebugDrawVectors.current.Clear ();
            DebugDrawVectors.current.groupAScale = 0.1f;
            DebugDrawVectors.current.groupBScale = 0.05f;
            DebugDrawVectors.current.groupAColor = Color.white;
            */
            int _branchDescriptorIndex = branchDescriptorIndex;
            branchDescriptorIndex = snapshotIndex;
            ProcessSnapshot(clearCompositeManager, materialMode, isNewSeed);
            // TODO: selectively reprocess.
            branchDescriptorIndex = _branchDescriptorIndex;
        }
        /// <summary>
        /// Regenerates a preview for the loaded snapshot.
        /// </summary>
        ...
        public void ProcessSnapshot(bool clearCompositeManager = true, MaterialMode materialMode = MaterialMode.Composite, bool isNewSeed = false)
        {
            ...
            if (materialMode == MaterialMode.Albedo)
            { // Albedo
            ...
            else if (materialMode == MaterialMode.Normals)
            { // Normals
            ...
            else if (materialMode == MaterialMode.Composite)
            { // Composite
        ...
        public void ProcessSnapshotPolygons(BranchDescriptor branchDescriptor)
        {
            ...
            */
            ...
            for (int lodLevel = branchDescriptor.lodCount - 1; lodLevel >= 0; lodLevel--)
            {
                GenerateSnapshotPolygonsPerLOD(lodLevel, branchDescriptor);
            }
            _refFragBounds.Clear();
