    ...
    public class CubicBezierCurve
    {
        ...
        */
        ...
        private void ApplyNoise(CurvePoint curvePoint)
        {
            ...
            float noiseSmoothness = 0.2f;
            float pointNoiseX =
                (curvePoint.lengthPosition + noiseLengthOffset + 0.5f) *
                Mathf.Lerp(noiseScaleAtFirstNode, noiseScaleAtLastNode, curvePoint.relativePosition);
            float pointNoiseY =
                (curvePoint.lengthPosition + noiseLengthOffset) *
                Mathf.Lerp(noiseScaleAtFirstNode, noiseScaleAtLastNode, curvePoint.relativePosition);
            Vector3 offset = new Vector3(
                (Mathf.PerlinNoise(pointNoiseX * noiseResolution, 0) - 0.5f) * noiseSmoothness,
                0f,
                (Mathf.PerlinNoise(0, pointNoiseY * noiseResolution) - 0.5f) * noiseSmoothness
            );
