...
namespace Artngame.CommonTools {
	...
	[CustomEditor(typeof(SplinerPInfiniGRASS))] 

	public class SplinePInfiniGRASSEditor : Editor {

		void Awake()
		{
			script = (SplinerPInfiniGRASS)target;
        ...
        #region PARAMETERS
        List<LandSplineNodeInfiniGRASS[]> Keep_Curve;
		//LandSplineNode[] Curve; //v2.1
		bool replot =true;
		private SplinerPInfiniGRASS script;
		...
		public void  OnSceneGUI () {
			...
			if (Keep_Curve == null | replot ){Keep_Curve = new List<LandSplineNodeInfiniGRASS[]>();}
			Event _Event = Event.current;
			EventType _Type = _Event.type;
            LandSplineNodeInfiniGRASS[]  SplinePoints = script.SplinePoints.ToArray();
				...
				if(script.control_points_children.Count !=0){
					...
					for (int i=0;i<script.SplinePoints.Count;i++){
						if(script.control_points_children[i] !=null){
							...
							if (script.SplinePoints[i].position == script.control_points_children[i].transform.position)
							{
								//Debug.Log("Spline change111 ");
								//do nothing
							}
							else{
								//Debug.Log("Spline change112 ");
		...
		void Draw_Spheres(int i, LandSplineNodeInfiniGRASS[]  SplinePoints, int Control_points_Count){
			if (script.PointGizmo_On) {Handles.color = Color.blue;
                ...
                Handles.SphereHandleCap(i, SplinePoints[i].position, Quaternion.identity, script.Handle_scale * 1f, EventType.Repaint);////v.
                if ( script.control_points_children != null){
					if(script.control_points_children.Count !=0){
						if( script.control_points_children[i] !=null){
							script.control_points_children[i].transform.localScale = (1/script.gameObject.transform.localScale.x)*0.3f*new Vector3(script.Handle_scale,script.Handle_scale,script.Handle_scale);
		...
		void Draw_Curve(int i, LandSplineNodeInfiniGRASS[]  SplinePoints, int Control_points_Count){
			if (i<=Control_points_Count-2){ 
				...
				if (Keep_Curve.Count<(i+1)|replot){
					...
					if(script.Curve1 != null & script.Keep_Curve != null){
						...
						script.Curve1 = new LandSplineNodeInfiniGRASS[1];	
						script.Keep_Curve = new List<LandSplineNodeInfiniGRASS[]>();
					}
					//				Curve = script.Curve1;
		...
		void ControlMe(LandSplineNodeInfiniGRASS[]  SplinePoints, int Control_points_Count){
			#region Curve Select
		...
		void Control_in(int i, LandSplineNodeInfiniGRASS[]  SplinePoints){
		...
		public override void  OnInspectorGUI() {
			...
			{
				script.SplinePoints.Add(new LandSplineNodeInfiniGRASS(script.gameObject.transform.position,Quaternion.identity));

				script.SplinePoints.Add(new LandSplineNodeInfiniGRASS(new Vector3(script.gameObject.transform.position.x,script.gameObject.transform.position.y+25,script.gameObject.transform.position.z),Quaternion.identity));
				script.SplinePoints.Add(new LandSplineNodeInfiniGRASS(new Vector3(script.gameObject.transform.position.x,script.gameObject.transform.position.y+50,script.gameObject.transform.position.z),Quaternion.identity));
				script.SplinePoints.Add(new LandSplineNodeInfiniGRASS(new Vector3(script.gameObject.transform.position.x,script.gameObject.transform.position.y+75,script.gameObject.transform.position.z),Quaternion.identity));
				script.SplinePoints.Add(new LandSplineNodeInfiniGRASS(new Vector3(script.gameObject.transform.position.x,script.gameObject.transform.position.y+100,script.gameObject.transform.position.z),Quaternion.identity));

				script.SplinePoints.Add(new LandSplineNodeInfiniGRASS(new Vector3(script.gameObject.transform.position.x,script.gameObject.transform.position.y+125,script.gameObject.transform.position.z),Quaternion.identity));
				script.SplinePoints.Add(new LandSplineNodeInfiniGRASS(new Vector3(script.gameObject.transform.position.x,script.gameObject.transform.position.y+150,script.gameObject.transform.position.z),Quaternion.identity));
				script.SplinePoints.Add(new LandSplineNodeInfiniGRASS(new Vector3(script.gameObject.transform.position.x,script.gameObject.transform.position.y+175,script.gameObject.transform.position.z),Quaternion.identity));
				script.SplinePoints.Add(new LandSplineNodeInfiniGRASS(new Vector3(script.gameObject.transform.position.x,script.gameObject.transform.position.y+200,script.gameObject.transform.position.z),Quaternion.identity));
			...
			if (GUILayout.Button ("Add another point")) {
				...
				script.SplinePoints.Add(new LandSplineNodeInfiniGRASS(Moved_vector_to_last_point,Quaternion.identity));
			...
			if(script.SplinePoints.Count > 2 ){
				if (GUILayout.Button ("Add point between")) {
					if(selectedSegment != -1){
						...
						script.SplinePoints.Insert(selectedSegment+1, new LandSplineNodeInfiniGRASS(Moved_vector_to_last_point,Quaternion.identity));
