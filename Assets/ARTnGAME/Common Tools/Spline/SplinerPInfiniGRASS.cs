...
//using Artngame.PANGAEA; //Assets.Scripts;
...
namespace Artngame.CommonTools
{
    ...
    public class SplinerPInfiniGRASS : MonoBehaviour
    {
        ...
        public bool splineChangedOut = false;
        ...
        public List<SplinerPInfiniGRASS> childrenSplines = new List<SplinerPInfiniGRASS>();
        ...
        public SplinerPInfiniGRASS parentSpline;
        ...
        public void AddPoint()
        {
            ...
            SplinePoints.Add(new LandSplineNodeInfiniGRASS(Moved_vector_to_last_point, Quaternion.identity));
        ...
        public void GrowVine()
        {
            ...
            while (EndCriteria == false)
            {
                ...
                if (Physics.Raycast(Origin, Direction, out hit, max_dist))
                {
                    ...
                    if (dist > moveStep)
                    { //if the distance is enough move ahead, otherwise move alone wall
        ...
        public List<LandSplineNodeInfiniGRASS[]> Keep_Curve;
        public LandSplineNodeInfiniGRASS[] Curve1;
        ...
        void Start()
        {
            ...
            #region CALC CURVE
            LandSplineNodeInfiniGRASS[] SplinePoints1 = SplinePoints.ToArray();
            ...
            if (Keep_Curve == null) { Keep_Curve = new List<LandSplineNodeInfiniGRASS[]>(); }
        ...
        void Draw_multithreaded(int i, LandSplineNodeInfiniGRASS[] SplinePoints1, int Control_points_Count, bool realtime)
        {
            float detail = CurveQuality;
            if (detail < 1)
            {
            ...
            List<LandSplineNodeInfiniGRASS> a = new List<LandSplineNodeInfiniGRASS>();
            ...
            for (float j = 0; j < 1; j += (1 / detail))
            {
                ...
                LandSplineNodeInfiniGRASS NEW_POINT = new LandSplineNodeInfiniGRASS(Vector3.zero, Quaternion.identity);
                NEW_POINT.position = p;
            ...
            {
                Curve1 = a.ToArray();
                List<LandSplineNodeInfiniGRASS> Curve_Temp = new List<LandSplineNodeInfiniGRASS>();
                Curve_Temp.AddRange(Curve1);
                Curve_Temp.Add(SplinePoints1[i + 1]);
                Curve1 = Curve_Temp.ToArray();
                Keep_Curve.Add(Curve1);
        ...
        //
        public void Draw_Curve(int i, LandSplineNodeInfiniGRASS[] SplinePoints1, int Control_points_Count, bool realtime)
        {
            //if ((Multithreaded && Thead_finished) | !Multithreaded) {
            //Curve.Clear ();
            //Curve.AddRange (Curve1);
        ...
        [SerializeField]
        public List<LandSplineNodeInfiniGRASS> Curve = new List<LandSplineNodeInfiniGRASS>();
        ...
        void Update()
        {
            ...
            if (Overide_seg_detail.Count < SplinePoints.Count - 1 | Keep_Overide_seg_detail.Count < SplinePoints.Count - 1)
            { //PDM v2.1
                Overide_seg_detail = new List<int>();
                for (int i = 0; i < SplinePoints.Count - 1; i++)
                {
                ...
                if (SplinePoints.Count > 2)
                {
                    ...
                    if (Add_mid_point)
                    {
                        ...
                        if (Add_in_Segment > 0 & Add_in_Segment < SplinePoints.Count)
                        {
                            ...
                            SplinePoints.Insert(Add_in_Segment, new LandSplineNodeInfiniGRASS(Moved_vector_to_last_point, Quaternion.identity));
                            GameObject SPHERE_SMALL = GameObject.CreatePrimitive(PrimitiveType.Sphere);
        ...
        [SerializeField()]
        public List<LandSplineNodeInfiniGRASS> SplinePoints = new List<LandSplineNodeInfiniGRASS>();
    ...
    #region Node
    [System.Serializable]
    public class LandSplineNodeInfiniGRASS
    {

        public LandSplineNodeInfiniGRASS(Vector3 pos, Quaternion rot)
        {
            this.position = pos;
            this.rotation = rot;
        }
