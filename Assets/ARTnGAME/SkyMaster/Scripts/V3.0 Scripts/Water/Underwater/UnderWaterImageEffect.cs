		...
		public class UnderWaterImageEffect : PostEffectsBaseSkyMaster {
			...
			private void WriteCoc ( RenderTexture fromTo, bool fgDilate) {
				...
				if (nearBlur && fgDilate) {
					...
					//v6.0
					RenderTexture tempfromToD = RenderTexture.GetTemporary (rtW, rtH, 0, fromTo.format);
					
					Graphics.Blit (fromTo, tempfromToD, dofHdrMaterial,  13);
					RenderTexture.ReleaseTemporary(temp2);
					
					Graphics.Blit (tempfromToD, fromTo);//v6.0
					RenderTexture.ReleaseTemporary(tempfromToD);//v6.0
				}
				else {
					...
					//v6.0
					RenderTexture tempfromToD = RenderTexture.GetTemporary (rtW, rtH, 0, fromTo.format);
					Graphics.Blit (fromTo,tempfromToD);
					Graphics.Blit (tempfromToD, fromTo, dofHdrMaterial,  0);
					
					//v6.0
					//Graphics.Blit (tempfromToD, fromTo);
					RenderTexture.ReleaseTemporary(tempfromToD);//v6.0
				}
			}
			...
			void OnRenderImage (RenderTexture source, RenderTexture destination) {
				...
				RenderTexture rtLowD = null;//v6.0
				...
				{					
					...
					rtLowD = RenderTexture.GetTemporary (source.width >> 1, source.height >> 1, 0, source.format);//v6.0
					...
					if (highResolution) {
						...
						// blur
						Graphics.Blit (source, rtLowD, dofHdrMaterial, 6);
												
						Graphics.Blit (rtLowD, rtLow);//v6.0
						
						//Graphics.Blit (rtLow, rtLow2, dofHdrMaterial, blurPass);
						Graphics.Blit (rtLow, rtLowD, dofHdrMaterial, blurPass);
						Graphics.Blit (rtLowD, rtLow2);//v6.0
				...
				if (rtLowD) RenderTexture.ReleaseTemporary(rtLowD);//v6.0
			}
		}
}
